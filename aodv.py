...
from ports import aodv_listener_ports, aodv_port, listener_ports, tester_port
import random

# Defines
AODV_HELLO_INTERVAL         =   2
AODV_HELLO_TIMEOUT          =   4
AODV_PATH_DISCOVERY_TIME    =   10
...
class aodv(threading.Thread):
    ...
    def __init__(self):
        ...
        self.status = "Active"
        self.hello_timer = 0
        self.probabilities_table = dict()
        self.neighbors_positions = dict()
    ...
    def aodv_send(self, destination, destination_port, message):
        try:
            if (self.probabilities_table[destination] < random.random()):
                return
    ...
    def aodv_add_neighbor(self, from_tester, neighborsList = None):
    ...
    def aodv_show_routing_table(self, from_tester):
        ...
        for r in self.routing_table.values():
            print("{:15} {:15} {:10} {:10} {:10}".format(
                r['Destination'], r['Next-Hop'], r['Seq-No'], r['Hop-Count'], r['Status']))
        print("")
    ...
    def aodv_get_active_routes(self):
        names = []
        for r in self.routing_table.values():
            if (r['Status'] == 'Active') and r['Destination'] == r['Next-Hop']:
                names.append(r['Destination'])
        return names
    
    def aodv_get_all_active_routes(self):
        names = []
        for r in self.routing_table.values():
            if (r['Status'] == 'Active'):
                names.append(r['Destination'])
    ...
    def aodv_get_routing(self):
        return self.routing_table
    
    def aodv_get_neighbors_positions(self):
        return self.neighbors_positions
    
    def aodv_get_inactive_routes(self):
        names = []
        for r in self.routing_table.values():
            if (r['Status'] == 'Inactive') and r['Destination'] == r['Next-Hop']:
                names.append(r['Destination'])
        return names
    
    def aodv_add_neighbors(self, neighborsList):
        neighbors = neighborsList
    ...
    def aodv_send_message_custom(self, source, dest, message_data):
        # Format the message
        ...
        if dest in self.routing_table.keys():
            ...
            if (destination['Status'] == 'Inactive'):
                ...
                # TODO: update lifetime here as the route was used
                self.aodv_restart_route_timer(destination, False)
                logging.debug("['CUSTOM_MESSAGE', '" + source + " to " + dest + " via " + next_hop + "', '" + message_data + "']")
        # else:
        #     # Initiate a route discovery message to the destination
        #     self.aodv_send_rreq(dest, -1)
    ...
    def aodv_process_custom_message(self, message):
        ...
        if (receiver == self.node_id):
            ...
            if neighbor in self.neighbors_positions.keys():
                last_time = int(msg[3])
                if last_time > self.neighbors_positions[neighbor][2]:
                    self.neighbors_positions[neighbor] = (msg[1], msg[2], last_time)
            else:
                self.neighbors_positions[neighbor] = (msg[1], msg[2], int(msg[3]))
            ...
            for r in readable:
                if r is self.listener_sock:
                    ...
                    if (message_type == "HELLO_MESSAGE"):
                        ...
                        self.aodv_process_custom_message(message)
